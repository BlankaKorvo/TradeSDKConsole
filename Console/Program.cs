...
namespace tradeSDK
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            var ccccc = await new GetTinkoffCandles("BBG000B9XRY4", (Tinkoff.Trading.OpenApi.Models.CandleInterval)CandleInterval.Day, DateTime.Now.AddYears(-10)).GetCandlesTinkoffAsync();
            stopwatch.Stop();
        ...
        //    var instrument = await GetMarketData.GetInstrumentByTickerAsync("AMZN");

        //    ICandlesList bigCandlesList = await GetMarketData.GetCandlesAsync(instrument.Figi, candleInterval, DateTime.Now.AddMonths(-13));
        ...
        //        IOrderbook orderbook = GetMarketData.GetOrderbookAsync(instrument.Figi, 50).GetAwaiter().GetResult();
        ...
        //                continue;
        //            }
        //            ICandlesList candleList = GetMarketData.GetCandlesAsync(instrument.Figi, candleInterval, candlesCount).GetAwaiter().GetResult();
        ...
        //private static void TestTrading(IOrderbook orderbook, ICandlesList candleList, ref TradeOperation tradeOperation, ref Portfolio.Position portfolioPosition, ref List<(decimal, decimal, decimal)> margin, bool realTime=true)
        //{
